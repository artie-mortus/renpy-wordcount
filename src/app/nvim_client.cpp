...
constexpr long kRpcTimeoutMs = 1500;
...
bool NvimClient::WriteMessage(const char* data, std::size_t size) {
    ...
    process_->GetOutputStream()->Write(data, size);
    return process_->GetOutputStream()->LastWrite() == size && process_->GetOutputStream()->IsOk();
}
...
std::optional<NvimClient::Value> NvimClient::Call(
    ...
    if (!WriteMessage(buffer.data, buffer.size)) {
        if (error) *error = "Could not write to Neovim.";
        msgpack_sbuffer_destroy(&buffer);
        return std::nullopt;
    }
    msgpack_sbuffer_destroy(&buffer);
    Value value;
    if (!ReadResponse(request_id, &value, error)) return std::nullopt;
    return value;
}
...
bool NvimClient::ReadResponse(std::uint64_t request_id, Value* value, std::string* error) {
    ...
    msgpack_unpacked_destroy(&unpacked);
    if (error) {
        *error = running() ? "Neovim did not answer within 1.5 seconds."
